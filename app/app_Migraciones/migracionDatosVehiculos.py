...
class DatosVehiculos():
    ...
    def __init__(self, envdds, envsql, archivotexto, encoding ):
        ...
        # we get el schema del environment DDS
        ...
        # we get el schema del environment SQL
    ...
    # >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    # verificacion, creacion , limpieza de los registros de las tablas de la 
    # lista de tablas
    ...
    def readTipoRegistro(self, **data):
        ...
        # Arma el query de busqueda para leer TIPOREGISTRO
        where = {
            'fieldnumber': [1, ],
    ...
    def readTipoSubRegistro(self, **data):
        """
         ...
         """
        # Arma el query de busqueda para leer TIPOSUBREGISTRO
        where = {
            'fieldnumber': [1, ],
            'field': [data['tiposubregistro'], ],
            'struct_query': ['fld', ],
            'op': ['EQ', ],
            'order': False,
            'pageno': False,
        ...
        data_list, errores = self.data_Input.get_rowsWhereWrk(self.objetoTipoSubRegistro, **where)
    ...
    def readTipoOrigen(self, **data):
        """
         ...
         """
        # Arma el query de busqueda para leer TIPOORIGEN
        where = {
            'fieldnumber': [1, ],
            'field': [data['tipoorigen'], ],
            'struct_query': ['fld', ],
            'op': ['EQ', ],
            'order': False,
            'pageno': False,
        ...
        data_list, errores = self.data_Input.get_rowsWhereWrk(self.objetoTipoOrigen, **where)
    ...
    def readProvincia(self, **data):
        ...
        if data['provincia'].isdigit():
            ...
            # Arma el query de busqueda para leer PROVINCIAS
            where = {
                'fieldnumber': [1, ],
            ...
            data_list, errores = self.data_Input.get_rowsWhereWrk(self.objetoProvincia, **where)
    ...
    def readTipoCuerpo(self, **data):
        ...
        'tipocuerpo'
        """
        # Arma el query de busqueda para leer TIPOCUERPO
        where = {
            'fieldnumber': [1, ],
            'field': [data['tipocuerpo'], ],
            'struct_query': ['fld', ],
            'op': ['EQ', ],
            'order': False,
            'pageno': False,
        ...
        data_list, errores = self.data_Input.get_rowsWhereWrk(self.objetoTipoCuerpo, **where)
    ...
    def readTipoDocumento(self, **data):
        ...
        """
        # Arma el query de busqueda para leer TIPODOCUMENTO
        where = {
            'fieldnumber': [1, ],
        ...
        data_list, errores = self.data_Input.get_rowsWhereWrk(self.objetoTipoDocumento, **where)
    ...
    def tipoRegistroC5Titular(self, id, registro):
        ...
        try:
            ...
            if not registro[180:220].isspace(): insertTitular['CALLE00001'] = unidecode.unidecode(registro[180:220].lstrip())[0:39]
            ...
            if not registro[230:240].isspace():  insertTitular['PISO_00001'] = unidecode.unidecode(registro[230:240].lstrip())[0:9]

            # -----DEPARTAMENTO------------------------------------------------------
            insertTitular['DEPAR00001'] = None
            if not registro[240:250].isspace(): insertTitular['DEPAR00001'] = unidecode.unidecode(registro[240:250].lstrip())[0:9]

            # -----BARRIO------------------------------------------------------
            insertTitular['BARRI00001'] = None
            if not registro[250:290].isspace(): insertTitular['BARRI00001'] = unidecode.unidecode(registro[250:290].lstrip())[0:39]

            # -----LOCALIDAD------------------------------------------------------
            insertTitular['LOCAL00001'] = None

            if not registro[290:330].isspace(): insertTitular['LOCAL00001'] = unidecode.unidecode(registro[289:330].lstrip())[0:39]
            ...
            if not registro[340:596].isspace(): insertTitular['RESER00001'] = unidecode.unidecode(registro[340:596].lstrip())[0:39]
